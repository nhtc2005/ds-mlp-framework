...
bool UNIT_TEST_Graph::graph08() {
    ...
    DGraphModel<char>* model =
        DGraphModel<char>::create(vertices, 4, edges, 5, &charComparator, &vertex2str);

    try {
        model->weight('A', 'D');
    } catch (const exception& e) {
        output << "Error: " << "Edge (E(A,D)) is not found" << endl;
    }

    try {
        model->weight('E', 'D');
    } catch (const exception& e) {
        output << "Error: " << "Vertex (E) is not found" << endl;
    }

    try {
        model->weight('D', 'F');
    } catch (const exception& e) {
        output << "Error: " << "Vertex (F) is not found" << endl;
    }
