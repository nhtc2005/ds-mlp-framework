...
string float_to_string(T* data, int nrows, int ncols, int width = 6, int prec = 2) {
    ...
    for (int r = 0; r < nrows; r++) {
        ...
        for (int c = 0; c < ncols; c++) {
            int index = r * stride + c;
            os << fixed << setw(width) << setprecision(prec) << data[index];
        }
        os << "]" << endl;
    }
    os << "]" << endl;
...
template <class T>
void int_uniform(T* data, int numel, T minValue = 0, T maxValue = 100, bool manualSeed = false,
                 int seedValue = 0) {
...
template <class T>
void real_uniform(T* data, int numel, T minValue = 0, T maxValue = 100, bool manualSeed = false,
                  int seedValue = 0) {
...
class Matrix {
   ...
   public:
    ...
    int numel() { return nrows * ncols; }
    int buffersize() { return nrows * ncols * itemsize(); }
    int itemsize() { return sizeof(T); }
    ...
    Matrix& uniform(double minValue = 0, double maxValue = 100, bool manualSeed = false,
                    int seedValue = 0, bool symetric = false) {
        if (this->dtype == INT32)
            int_uniform<int>((int*)(this->data), this->numel(), minValue, maxValue, manualSeed,
                             seedValue);
        if (this->dtype == INT8)
            int_uniform<int8>((int8*)(this->data), this->numel(), minValue, maxValue, manualSeed,
                              seedValue);
        if (this->dtype == UINT8)
            int_uniform<uint8>((uint8*)(this->data), this->numel(), minValue, maxValue, manualSeed,
                               seedValue);
        if (this->dtype == FLOAT32)
            real_uniform<float>((float*)(this->data), this->numel(), minValue, maxValue, manualSeed,
                                seedValue);
        if (this->dtype == FLOAT64)
            real_uniform<double>((double*)(this->data), this->numel(), minValue, maxValue,
                                 manualSeed, seedValue);
    ...
    Matrix& uniformsym(double minValue = 0, double maxValue = 100, bool manualSeed = false,
                       int seedValue = 0, bool symetric = false) {
        if (this->dtype == INT32)
            int_uniform<int>((int*)(this->data), this->numel(), minValue, maxValue, manualSeed,
                             seedValue);
    ...
    Matrix& binomial(float p = 0.5, bool manualSeed = false, int seedValue = 0,
                     bool symetric = false) {
    ...
    void println(int width = 6, int prec = 2) { cout << toString(width, prec) << endl; }
    string toString(int width = 6, int prec = 2) {
        ...
        if (this->dtype == FLOAT32)
            return float_to_string<float>((float*)(this->data), this->nrows, this->ncols, width,
                                          prec);
        if (this->dtype == FLOAT64)
            return float_to_string<double>((double*)(this->data), this->nrows, this->ncols, width,
                                           prec);
        return "dtype: not supported\n";
    }
};

int* genIntArray(int size, int minValue = 0, int maxValue = 100, bool manualSeed = false,
                 int seedValue = 0) {
...
int genInt(int minValue = 0, int maxValue = 100, bool manualSeed = false, int seedValue = 0) {
    int value;
    uniform_int_distribution<int> dist(minValue, maxValue);
